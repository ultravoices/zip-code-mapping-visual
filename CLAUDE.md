...
- No state mgmt lib — local React state only
- No tests
...
**TIGER API** — boundary data from Census Bureau REST API:
...
**Zip batching** — `useZipBoundaries` collect cache-missing zips, issue single TIGER `IN (zip1, zip2, ...)` query. No batch size limit (safe ~120 zips, fragile beyond ~300).

**Municipality spatial filter** — `useCountyData` ray-cast point-in-polygon on TIGER interior points (INTPTLAT/INTPTLON), exclude adjacent-county bleed. Missing centroid = include (conservative).
